...
def model_output_app(model, dataloader, export_prob=True, export_feat=True, softmax=True, device=None,
                     feature_layer='pool', progress_bar=True,
                     export_crop_pos=False):
    """
    ...
    for ibatch, sample in enumerate(iter(dataloader)):
        ...
        if export_crop_pos:
            lcrop.append(list(zip(sample['crop'][0].numpy(), sample['crop'][1].numpy())))
        if progress_bar:
            pbar.update(1)
    ...
    # 4) Frame with crop position
    if export_crop_pos:
        lcrop = np.vstack(lcrop)
        colnames = ['Crop_start', 'Crop_end']
        frames.append(pd.DataFrame(lcrop, columns=colnames))
